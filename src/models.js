...
const _modelPermissionsConfig = { properties: {}, permissions: {}, model: {}, args: []}
...
const models = ({
  ...
  const Users = Model(MODEL_NAMES.Users, {
    firstName: NameProperty({ required: true }),
    lastName: NameProperty({ required: true }),
    email: EmailProperty(ormPropertyConfig({ required: true, unique: 'email' })),
    roles: RolesProperty(rolesArray, { ...usersConfig.roles }),
    ...usersConfig.properties
  }, ...usersConfig.args)

  const ModelPermissions = Model(MODEL_NAMES.ModelPermissions, {
    model: TextProperty(merge({}, modelPermissionsConfig.model, ormPropertyConfig({ required: true, unique: true }))),
